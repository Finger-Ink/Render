...
open class ComponentView<S: StateType>: UIView, ComponentViewType {
	...
	private func internalRender(in bounds: CGSize = CGSize.max, options: [RenderOption]) {
		...
		func layout() {
			...
			contentView.frame.size.height += normalize(yoga.marginTop) + normalize(yoga.marginBottom)
			contentView.frame.size.width += normalize(yoga.marginLeft) + normalize(yoga.marginRight)
		...
		if let animation = RenderOption.first(opts, .__animated) {
			...
			let newViews: [(UIView, CGFloat)] = views { view in
				view.isNewlyCreated && !RenderOption.contains(opts, .preventViewHierarchyDiff)
			}.map { view in
			...
			switch animation {
			case let .animated(duration, options, alongside):
				if #available(iOS 10, *) {
	...
	private func reconcile(new: NodeType, size: CGSize, view: UIView?, parent: UIView) {
		...
		for subnode in new.children {
			...
			let candidateView = oldSubviews?.filter { view in
				view.tag == subnode.identifier.hashValue
			}.first
