...
class TodoReducer: Reducer<TodoListState, Action> {
	...
	override func operation(for action: Action, in store: S) -> O {
		...
		switch action {
		case .add:
			return ActionOperation(action: action, store: store, block: add)

		case .name:
			return ActionOperation(action: action, store: store, block: name)

		case .clear:
			return ActionOperation(action: action, store: store, block: clear)

		case .check:
			return ActionOperation(action: action, store: store, block: check)
		}
	}
	...
	private func name(operation: AsynchronousOperation, action: Action, store: S) {
		...
		guard case let .name(id, title) = action else {
			return
		}
		store.updateState { state in
	...
	private func check(operation: AsynchronousOperation, action: Action, store: S) {
		...
		guard case let .check(id) = action else {
			return
		}
		store.updateState { state in
...
extension Dispatcher {
	...
	var todoListStore: Store<TodoListState, Action> {
		return store(with: Dispatcher.todoListStoreIndetifier) as! Store<TodoListState, Action>
	}
